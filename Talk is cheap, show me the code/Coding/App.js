    ...
    return(
      <div className='card-container'>
        {restaurantList.map((resto)=>(
          <RestronantCard data = {resto.data}/>
        ))}
      </div>
    )
  }
  const RestronantCard =({data})=>{
    const{cloudinaryImageId, name, cuisines, costForTwoString, deliveryTime} = data
    return(
      <div className='resto-card'>
          <img alt='image' src={`https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660/${cloudinaryImageId}`} />
         <div className='info'>
  ...
  const Footer = ()=>{
    return(
      <div className='footer'>
          <p>All Rights Reserved by Hungray</p>
      </div>
    )
  }
